...
void DefineObstacle(int* matrix, int row, int column, int x_axis, int y_axis, Obstacle* obs) {
    if (x_axis - 1 < 0) {
        obs->up = 0;
    } else if (matrix[(x_axis-1)*column + y_axis] == 1 || matrix[(x_axis-1)*column + y_axis] == 3) {
        obs->up = 0;
    } else {
        obs->up = 1;
    }
    ...
    if (y_axis + 1 < 0 || y_axis + 1 >= column) {
        obs->right = 0;
    } else if (matrix[x_axis*column + y_axis+1] == 1 || matrix[x_axis*column + y_axis+1] == 3) {
        obs->right = 0;
    } else {
        obs->right = 1;
    }
...
void CheckObstacle(int* matrix, int row, int column, int x_axis, int y_axis, Obstacle* obs) {
    ...
    printf("\nLocation: \033[1;33m(%d, %d)\033[0m\nObstacle: \033[1;35mup:%d down:%d left:%d right:%d\033[0m\n",
           x_axis, y_axis,
